...
- **SEMPRE** use `z.enum(WeekDay)` importado de `../generated/prisma/enums.js` para tipar campos de dia da semana nos schemas (request, response e schemas inline nas rotas). **NUNCA** use `z.string()` para representar WeekDay.
- O `OutputDto` dos use cases também deve usar o tipo `WeekDay` (importado de `../generated/prisma/enums.js`) para campos de dia da semana. **NUNCA** use `string` para representar WeekDay.
